...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("new_order_form"):
        ...
        num_items = st.number_input("How many item types?", min_value=1, max_value=10, value=1)
        ...
        for i in range(int(num_items)):
            ...
            with c3:
                ...
                unit_price = st.number_input(
                    f"Price (₹) [default: {default_price}]",
                    min_value=0.0, value=float(default_price),
                    key=f"price_{i}"
                )
            ...
            with st.expander(f"📏 Measurements for {item_type} {i+1}"):
                meas = {}
                if item_type in ["Kurti", "Blouse", "Anarkali", "Gown", "Crop Top", "Jacket", "Saree Blouse"]:
                ...
                elif item_type in ["Salwar", "Churidar", "Pant", "Palazzo", "Lehenga", "Sharara", "Skirt"]:
            ...
            items_data.append({"item_type": item_type, "quantity": int(qty), "unit_price": unit_price, "measurements": meas})

        st.markdown("---")
        st.markdown(f"**Total Amount Preview: ₹{total_preview:,.0f}**")
        ...
        if submitted:
            if not cust_name:
                ...
                if result and "id" in result:
                    ...
                    st.balloons()
                    st.info(f"**Total: ₹{float(result['total_amount']):,.0f}** | Advance: ₹{float(result['advance_paid']):,.0f} | Remaining: ₹{float(result['remaining_amount']):,.0f}")
